...
CREATE DATABASE db_Project_LibraryManagementSystem
GO
USE db_Project_LibraryManagementSystem
GO
...
(@BooktTitle varchar(max) = 'The Lost tribe',@BranchName varchar(max) = 'Sharpstown')
AS 
SELECT tbl_library_branch.library_branch_BranchID,tbl_library_branch.library_branch_BranchName,tbl_book_copies.book_copies_CopiesID 
FROM tbl_book_copies	INNER JOIN tbl_book ON tbl_book_copies.book_copies_BookID = tbl_book.book_bookID 
						INNER JOIN tbl_library_branch ON tbl_book_copies.book_copies_BranchID = tbl_library_branch.library_branch_BranchID
WHERE book_Title =@BooktTitle AND library_branch_BranchName =@BranchName
GO
...
CREATE PROC dbo.BookCopiesAtAllBranchs
(@BookTitle varchar(max)= 'It')
AS 
SELECT tbl_library_branch.library_branch_BranchID,tbl_library_branch.library_branch_BranchName,tbl_book_copies.book_copies_CopiesID 
FROM tbl_book_copies	
			INNER JOIN tbl_book ON tbl_book_copies.book_copies_BookID = tbl_book.book_bookID
			INNER JOIN tbl_library_branch ON tbl_book_copies.book_copies_BranchID = tbl_library_branch.library_branch_BranchID
WHERE tbl_book.book_Title = @BookTitle
GO

EXEC dbo.BookCopiesAtAllBranchs
----------------------------------------------
...
FROM tbl_borrower
WHERE NOT EXISTS	(SELECT * FROM tbl_book_loans
...
CREATE PROC dbo.LoansInfo
(@DueDate date = '2/2/18', @LibraryBranchName varchar(MAX) ='Sharpstown')
AS
SELECT tbl_library_branch.library_branch_BranchName AS [Branch Name],  tbl_book.book_Title [Book Name],
	   tbl_borrower.borrower_BorrowerName AS [Borrower Name], tbl_borrower.borrower_BorrowerAddress AS [Borrower Address],
	   tbl_book_loans.book_loans_DateOut AS [Date Out], tbl_book_loans.book_loans_DueDate [Due Date]
FROM tbl_book_loans
...
WHERE tbl_book_loans.book_loans_DueDate = @DueDate AND tbl_library_branch.library_branch_BranchName = @LibraryBranchName
GO 
EXEC dbo.LoansInfo
...
AS
SELECT tbl_library_branch.library_branch_BranchName, COUNT(tbl_book_loans.book_loans_BranchID) AS TOTAL_LOANS
FROM tbl_book_loans INNER JOIN tbl_library_branch ON  tbl_book_loans.book_loans_BranchID = tbl_library_branch.library_branch_BranchID 
